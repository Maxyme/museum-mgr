...
async def test_create_user(
    authenticated_test_client: httpx.AsyncClient, admin_user_id: str
):
    ...
    user = ApiUserOut.model_validate(response.json())
...
async def test_list_users(
    ...
    users = [ApiUserOut.model_validate(u) for u in response.json()]
...
async def test_update_user(
    ...
    new_user_data = UserCreateFactory.build(is_admin=True)
    create_response = await authenticated_test_client.post(
        "/users", json=new_user_data.model_dump()
    )
    new_user_id = create_response.json()["id"]

    update_data = ApiUserIn(
        name="Updated Name", email=new_user_data.email, is_admin=True
    )
    response = await authenticated_test_client.patch(
        f"/users/{new_user_id}", json=update_data.model_dump(exclude_unset=True)
    )
    assert response.status_code == 200
    updated_user = ApiUserOut.model_validate(response.json())
